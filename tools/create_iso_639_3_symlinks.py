...
repo_re=re.compile(r"^UD_(.*?)(-(.*))?$") #matches UD_Ancient_Greek-PROIEL $1 -> Ancient_Greek  $3 -> PROIEL
file_re=re.compile(r"^([a-z]{2,3})(_(.*))?-ud-((train|dev|test).*)\.conllu$") #matches gr_proiel-ud-train.conllu
if __name__=="__main__":
    ...
    for repo in all_repos:
        match=repo_re.match(os.path.basename(repo))
        if not match:
            print("# note: {} does not look like a UD repository, skipping", file=sys.stderr)
            continue
        lname=match.group(1).replace("_"," ")  #Ancient Greek
        tbank=match.group(3) if match.group(3) is not None else ""  #PROIEL or ""

        files=glob.glob(os.path.join(repo,"*-ud-*.conllu")) #all files which look like they could be part of UD
        if not files:
            continue
        
        codes=set()
        files_to_link=[] #[(fname, match_obj),...]
        for f in files:
            match=file_re.match(os.path.basename(f))
            if not match:
                print("# note: {} does not seem to fit the naming convention. Skipping.".format(f),file=sys.stderr) #skip what doesn't match the exact pattern of train/test/dev
                continue
            else:
                codes.add(match.group(1))
                files_to_link.append((f,match))

        if not files_to_link: #empty treebank
            continue

        assert len(codes)==1, "Repo {} has multiple language codes!".format(repo)
        lcode=codes.pop() #en
        if len(lcode)==2:
            l3code=c2to3[lcode] #two char to three char
        else:
            l3code = lcode
        if c3_to_name[l3code]!=lname: #check that name of language in the repo matches with ISO
            print("# note: assuming '{}' in UD and '{}' in ISO 639-3 are same language".format(lname,c3_to_name[l3code]),file=sys.stderr)
        if tbank:
            lcode+="-"+tbank.lower() #grc-proiel
        
        target=os.path.abspath(os.path.join(args.TARGETDIR[0],lcode))
